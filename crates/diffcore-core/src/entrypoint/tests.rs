...
fn test_detect_nextjs_page_tsx() {
    ...
    assert!(result
        .iter()
        .any(|e| e.symbol == "DashboardPage" && e.entrypoint_type == EntrypointType::ReactPage));
}
...
fn test_detect_effect_queue_take() {
    ...
    assert!(result.iter().any(
        |e| e.symbol == "processMessages" && e.entrypoint_type == EntrypointType::QueueConsumer
    ));
}
...
fn test_detect_effect_pubsub_subscribe() {
    ...
    assert!(result
        .iter()
        .any(|e| e.symbol == "handleEvents" && e.entrypoint_type == EntrypointType::QueueConsumer));
}
...
fn test_detect_effect_stream_run() {
    ...
    assert!(result
        .iter()
        .any(|e| e.symbol == "processStream" && e.entrypoint_type == EntrypointType::EventHandler));
}
...
fn test_detect_effect_hub_subscribe() {
    ...
    let result = detect_entrypoints(&[file]);
    assert!(result.iter().any(
        |e| e.symbol == "listenForEvents" && e.entrypoint_type == EntrypointType::EventHandler
    ));
}
...
fn test_detect_effect_stream_subpath_import() {
    ...
    assert!(result
        .iter()
        .any(|e| e.symbol == "drainEvents" && e.entrypoint_type == EntrypointType::EventHandler));
}
...
fn test_detect_effect_service() {
    ...
    assert!(result
        .iter()
        .any(|e| e.symbol == "UserService" && e.entrypoint_type == EntrypointType::EffectService));
}
...
fn test_detect_context_tag() {
    ...
    let result = detect_entrypoints(&[file]);
    assert!(result.iter().any(
        |e| e.symbol == "DatabaseService" && e.entrypoint_type == EntrypointType::EffectService
    ));
}
...
fn test_detect_layer_succeed() {
    ...
    assert!(result
        .iter()
        .any(|e| e.symbol == "LiveLayer" && e.entrypoint_type == EntrypointType::EffectService));
}
...
fn test_detect_layer_scoped() {
    ...
    let result = detect_entrypoints(&[file]);
    assert!(result.iter().any(
        |e| e.symbol == "ConnectionLayer" && e.entrypoint_type == EntrypointType::EffectService
    ));
}
